...
mkdir -p "$BARE_AI_DIR/diary" "$BARE_AI_DIR/logs"
...
echo -e "$README_CONTENT" > "$BARE_AI_DIR/README.md"
...
if ! grep -q "color_prompt=yes" "$BASHRC_FILE" || ! grep -q "force_color_prompt=yes" "$BASHRC_FILE"; then
    ...
    echo -e "\n$BASHRC_COLOR_SETTINGS" >> "$BASHRC_FILE"
    if [ $? -ne 0 ]; then
        echo -e "${RED}Error: Failed to add color prompt settings to $BASHRC_FILE. Exiting.${NC}"
        exit 1
    fi
    echo -e "${YELLOW}Terminal color prompt settings added. Please run 'source $BASHRC_FILE' to apply.${NC}"
else
